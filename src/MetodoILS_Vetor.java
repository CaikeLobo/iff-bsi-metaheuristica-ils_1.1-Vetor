...
public class MetodoILS_Vetor {
    ...
    public MetodoILS_Vetor(Mochila mochila, int criterioDeParadaDoILS, int criterioDeParadaDaBuscaLocal, int tamanhoMaximoPertubação) {
    ...
    private int verificarMochila(int[] solucao) {
        ...
        for (int i = 0; i < solucao.length; i++) {
            if (solucao[i] == 1) {
                pesoTotal += this.mochila.getItemV(i).peso;
                valorTotal += this.mochila.getItemV(i).valor;
            }
        }
    ...
    public int[] encontrarSolucao() {
    	 ...
    	 */
        int[] solucaoAtual = new int[this.mochila.getTotalItens()];
        ...
        for (int i = 0; i < criterioDeParadaDoILS; i++) {
            ...
            if (verificarMochila(novaSolucao) > verificarMochila(melhorSolucao)) {
                melhorSolucao = novaSolucao;
                System.out.println(" Nova Solução Encontrada!");
            }
    ...
    public final void exibirSolucao(int[] solucao) {
        ...
        for (int i = 0; i < solucao.length; i++) {
            if (solucao[i] == 1) {
                Item item = this.mochila.getItemV(i);
